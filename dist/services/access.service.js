...
exports.getPermissionsForMembership = getPermissionsForMembership;
...
exports.listAssignableEventDates = listAssignableEventDates;
...
function uniqueKnownPermissions(permissionKeys) {
    return Array.from(new Set(permissionKeys
        .map((permission) => permission.trim())
        .filter(Boolean)
        .filter(permissions_1.isKnownPermission)));
}
function getPermissionCatalog() {
    return permissions_1.PERMISSION_CATALOG;
}
async function userHasPermission(input) {
    ...
    if (membership.systemRole === 'ADMIN')
        return true;
    return (membership.customRole?.active === true &&
        membership.customRole.permissions.some((permission) => permission.permissionKey === input.permission));
}
async function getPermissionsForMembership(input) {
    const membership = await prisma_1.prisma.userCompany.findUnique({
        where: {
            userId_companyId: {
                userId: input.userId,
...
async function updateRole(input) {
    ...
    const nextPermissionKeys = typeof input.permissionKeys === 'undefined'
...
async function getMembershipForResponse(tx, membershipId) {
...
async function listCompanyUsers(companyId) {
    return prisma_1.prisma.userCompany.findMany({
        where: { companyId },
        include: {
            user: {
                select: {
...
async function createCompanyUser(input) {
    ...
    const membership = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        const createdMembership = user.memberships.find((membership) => membership.companyId === input.companyId);
...
async function listAssignableEventDates(companyId) {
    const now = new Date();
    const from = new Date(now);
    from.setMonth(from.getMonth() - 2);
    const to = new Date(now);
    to.setMonth(to.getMonth() + 6);
    return prisma_1.prisma.eventDate.findMany({
        where: {
            companyId,
...
async function updateMyActiveEventDate(input) {
    const activeEventDateId = await resolveActiveEventDateId({
        companyId: input.companyId,
        activeEventDateId: input.activeEventDateId ?? null,
    });
...
async function ensureAnotherAdminWillRemain(input) {
    ...
    if (membership.systemRole !== 'ADMIN')
        return;
    const adminCount = await prisma_1.prisma.userCompany.count({
        where: {
            companyId: input.companyId,
            systemRole: 'ADMIN',
    ...
    if (adminCount <= 0)
        throw new Error('LAST_ADMIN_CANNOT_BE_CHANGED');
}
...
async function updateMembershipAccess(input) {
    const existing = await prisma_1.prisma.userCompany.findFirst({
        where: { id: input.membershipId, companyId: input.companyId },
...
async function updateCompanyUser(input) {
    const existing = await prisma_1.prisma.userCompany.findFirst({
        where: { id: input.membershipId, companyId: input.companyId },
        include: {
    ...
    if (username !== undefined) {
        ...
        if (username !== existing.user.username) {
            const duplicated = await prisma_1.prisma.user.findUnique({ where: { username }, select: { id: true } });
            if (duplicated)
                throw new Error('USERNAME_ALREADY_EXISTS');
        }
    }
