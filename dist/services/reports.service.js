...
function getUnitContentBaseQuantity(product) {
    if (!hasUnitContent(product))
        return 1;
 ...
 */
function getStoredUnitCost(product) {
 ...
 */
function getIngredientBaseUnitCost(product) {
    const referenceCost = toNumber(product?.referenceCost);
    const referenceQuantity = toNumber(product?.referenceQuantity);
    const simpleCost = toNumber(product?.simpleCost);
...
function getRecipeOutputBaseQuantity(product) {
...
function getRecipeCostForFactor(product, factor, seen = new Set()) {
    if (!product || !Number.isFinite(factor) || factor <= 0)
        return 0;
    if (!productHasRecipe(product))
        return getStoredUnitCost(product) * factor;
    if (seen.has(product.id))
        return 0;
    const nextSeen = new Set(seen);
    nextSeen.add(product.id);
    return (product.recipeItems ?? []).reduce((sum, recipeItem) => {
        const ingredient = recipeItem.ingredientProduct;
        const quantity = toNumber(recipeItem.quantity) * factor;
        if (!ingredient || quantity <= 0)
            return sum;
        return (sum +
            getProductCostForIngredientQuantity(ingredient, quantity, recipeItem.unit, nextSeen));
    }, 0);
}
/**
 ...
 */
function getProductCostForIngredientQuantity(product, quantity, unit, seen = new Set()) {
    if (!product || !Number.isFinite(quantity) || quantity <= 0)
        return 0;
    if (productHasRecipe(product) || product.costMode === 'recipe' || product.madeOnDemand) {
        if (seen.has(product.id))
            return 0;
        const requestedBaseQuantity = normalizeToBaseUnit(quantity, unit ?? product.recipeOutputUnit ?? getEffectiveStockUnit(product));
 ...
 */
function getProductCostForOrderQuantity(product, orderQuantity) {
    if (!product || !Number.isFinite(orderQuantity) || orderQuantity <= 0)
        return 0;
    if (productHasRecipe(product) || product.costMode === 'recipe' || product.madeOnDemand) {
...
function getVariationOptionCostForOrderQuantity(option, orderQuantity) {
    if (!option || !Number.isFinite(orderQuantity) || orderQuantity <= 0)
        return 0;
    if (option.costMode !== 'recipe' && !(option.recipeItems?.length)) {
        return getStoredUnitCost(option) * orderQuantity;
    }
    return (option.recipeItems ?? []).reduce((sum, recipeItem) => {
        const ingredient = recipeItem.ingredientProduct;
        const quantity = toNumber(recipeItem.quantity) * orderQuantity;
        if (!ingredient || quantity <= 0)
            return sum;
        return (sum +
            getProductCostForIngredientQuantity(ingredient, quantity, recipeItem.unit));
    }, 0);
}
function estimateOrderItemCost(item) {
...
function estimateStaffCost(eventDate) {
    return (eventDate.people ?? [])
        ...
        if (rateType === 'HOURLY') {
            ...
            const eventHours = end > start ? (end - start) / 3600000 : 0;
...
function estimateEventBuyCost(eventDate) {
...
function getOrderEnvironment(order) {
...
function monthPeriodSort(label) {
    if (label === 'Início do mês')
        return 1;
    if (label === 'Meio do mês')
        return 2;
    if (label === 'Fim do mês')
        return 3;
    return 99;
...
function buildWhere(companyId, filters) {
    ...
    if (filters.eventDateId && filters.eventDateId !== 'all')
        where.eventDateId = filters.eventDateId;
    if (filters.customerId && filters.customerId !== 'all')
        where.customerId = filters.customerId;
    if (filters.internalCustomerId && filters.internalCustomerId !== 'all')
        where.internalCustomerId = filters.internalCustomerId;
    if (filters.comanda?.trim()) {
        const comanda = Number(filters.comanda);
        if (Number.isFinite(comanda))
            where.comanda = comanda;
    }
    if (filters.taxApplied === 'true')
        where.taxApplied = true;
    if (filters.taxApplied === 'false')
        where.taxApplied = false;
...
async function getReportFilters(companyId) {
    const [events, environments, categories, products, customers, internalCustomers] = await Promise.all([
        ...
        prisma_1.prisma.salesEnvironment.findMany({
        ...
        prisma_1.prisma.internalCustomer.findMany({
            where: { companyId, active: true },
            orderBy: { name: 'asc' },
...
async function getReportsDashboard(companyId, filters) {
    ...
    const bestHourByDayMap = new Map();
    const bestDayByWeekMap = new Map();
    ...
    for (const order of orders) {
        ...
        addMetricRow(monthPeriodMap, monthPeriod, { period: monthPeriod, orders: 0, paidOrders: 0, revenue: 0, cost: 0, profit: 0, averageTicket: 0 }, orderRevenue, orderCost, order.status);
    ...
    const bestHourByDayMapReduced = [...bestHourByDayMap.values()].reduce((map, row) => {
        const current = map.get(row.date);
        if (!current || row.revenue > Number(current.revenue ?? 0))
            map.set(row.date, row);
        return map;
    }, new Map());
    const bestHourByDay = [...bestHourByDayMapReduced.values()].sort((a, b) => String(a.date).localeCompare(String(b.date)));
    const bestDayByWeekMapReduced = [...bestDayByWeekMap.values()].reduce((map, row) => {
        const current = map.get(row.weekStart);
        if (!current || row.revenue > Number(current.revenue ?? 0))
            map.set(row.weekStart, row);
        return map;
    }, new Map());
    const bestDayByWeek = [...bestDayByWeekMapReduced.values()].sort((a, b) => String(a.weekStart).localeCompare(String(b.weekStart)));
    ...
    function normalizeMoneyObject(item) {
        const next = { ...item };
        for (const key of Object.keys(next)) {
            if ([
                ...
                next[key] = round(Number(next[key] ?? 0), key === 'marginPercent' ? 1 : 2);
            }
        }
        return next;
    }
        ...
        tables: {
            ...
            recentOrders: orders.slice(0, 50).map((order) => ({
                ...
                items: order.items.map((item) => ({
                    ...
                    variations: (item.variations ?? []).flatMap((variation) => (variation.options ?? []).map((selectedOption) => {
