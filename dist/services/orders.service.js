...
exports.generateOrdersReportPdf = generateOrdersReportPdf;
...
function getStockBaseQuantity(product, fallbackUnit) {
    ...
    return normalizeToBaseUnit(stockQuantity, product?.stockUnit ?? fallbackUnit ?? 'unit');
}
function convertBaseQuantityBackToStoredStock(baseQuantity, product, fallbackUnit) {
    if (hasUnitContent(product)) {
...
async function decrementIngredientStock(tx, ingredientProductId, quantity, unit) {
    ...
    if (!ingredient)
        return;
    if (!ingredient.trackStock || ingredient.unlimitedStock)
        return;
    ...
    if (!Number.isFinite(decrementInBase) ||
        decrementInBase <= 0 ||
        !Number.isFinite(currentStockBase)) {
        return;
    }
    const nextStockBase = currentStockBase - decrementInBase;
    const nextStockQuantity = convertBaseQuantityBackToStoredStock(nextStockBase, ingredient, unit);
    await tx.product.update({
        where: { id: ingredientProductId },
        data: {
            stockQuantity: new client_1.Prisma.Decimal(nextStockQuantity),
        },
    });
}
async function consumeProductRecipe(tx, productId, requiredBaseQuantity, requiredBaseUnit) {
    ...
    const outputUnit = product.recipeOutputUnit ??
    ...
    if (product.madeOnDemand &&
        ...
        for (const recipeItem of recipeItems) {
            ...
            if (!Number.isFinite(nestedRequired) || nestedRequired <= 0)
                continue;
            const nestedBaseQuantity = normalizeToBaseUnit(nestedRequired, recipeItem.unit);
            await consumeProductRecipe(tx, recipeItem.ingredientProductId, nestedBaseQuantity, recipeItem.unit);
        }
        return;
    }
    const stockUnit = getEffectiveStockUnit(product, requiredBaseUnit);
    const storedQuantityToDecrement = stockUnit === 'l' || stockUnit === 'kg'
        ? requiredBaseQuantity / 1000
        : requiredBaseQuantity;
    await decrementIngredientStock(tx, productId, storedQuantityToDecrement, stockUnit);
}
async function incrementIngredientStock(params) {
    ...
    if (!product)
        return;
    if (!product.trackStock || product.unlimitedStock)
        return;
    ...
    if (!Number.isFinite(incrementInBase) ||
        incrementInBase <= 0 ||
        !Number.isFinite(currentStockBase)) {
        return;
    }
    const nextStockBase = currentStockBase + incrementInBase;
    const nextStockQuantity = convertBaseQuantityBackToStoredStock(nextStockBase, product, unit);
...
async function restoreProductRecipe(params) {
    const { tx, companyId, userId, productId, requiredBaseQuantity, requiredBaseUnit, reason, } = params;
    const product = await tx.product.findFirst({
        where: {
            id: productId,
            companyId,
    ...
    const stockUnit = getEffectiveStockUnit(product, requiredBaseUnit);
    const stockQuantity = stockUnit === 'l' || stockUnit === 'kg'
        ? requiredBaseQuantity / 1000
        : requiredBaseQuantity;
...
async function applyStockFromOrder(tx, orderId, companyId) {
    const order = await tx.order.findFirst({
        where: {
            id: orderId,
            companyId,
    ...
    for (const item of order.items) {
        ...
        const soldProductBase = itemQuantity * productOutputBase;
        if (item.product.madeOnDemand && (item.product.recipeItems?.length ?? 0) > 0) {
            await consumeProductRecipe(tx, item.productId, soldProductBase, productOutputUnit);
        }
        else if (item.product.trackStock && !item.product.unlimitedStock) {
            ...
            for (const selected of variation.options ?? []) {
                for (const recipeItem of selected.option.recipeItems ?? []) {
                    ...
                    if (usedQuantity <= 0)
                        continue;
                    const usedBase = normalizeToBaseUnit(usedQuantity, recipeItem.unit);
                    await consumeProductRecipe(tx, recipeItem.ingredientProductId, usedBase, recipeItem.unit);
                }
            }
        }
    }
}
async function paySelectedInternalCustomerOrders(input) {
...
async function createOrder(data, userId) {
    ...
    if (!resolvedCustomerId && data.eventDateId && data.comanda != null) {
        const eventComanda = await prisma_1.prisma.eventCustomerComanda.findFirst({
            where: {
                companyId: data.companyId,
    ...
    const validPaymentMethods = ['money', 'pix', 'credit', 'debit'];
    if (data.status === 'paid') {
    ...
    if (data.internalCustomerId) {
        const internalCustomer = await prisma_1.prisma.internalCustomer.findUnique({
            where: {
    ...
    const order = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        const shouldConsumeStock = data.status === 'paid' ||
...
async function cancelOrder(orderId, companyId, userId) {
    ...
    const order = await prisma_1.prisma.$transaction(async (tx) => {
        const shouldRestock = existingOrder.status === 'paid' ||
...
async function getInternalCustomerPendingOrders(companyId, internalCustomerId) {
    const customer = await prisma_1.prisma.internalCustomer.findFirst({
        where: {
...
async function getInternalCustomerTodayOrders(companyId, internalCustomerId) {
...
async function payInternalCustomerTodayOrders(input) {
    const pendingOrders = await prisma_1.prisma.order.findMany({
        where: {
            companyId: input.companyId,
            internalCustomerId: input.internalCustomerId,
...
async function generateOrdersReportPdf(companyId) {
    const orders = await prisma_1.prisma.order.findMany({
        where: { companyId },
    ...
    return await new Promise((resolve, reject) => {
        ...
        for (const order of orders) {
            ...
            doc.moveDown(0.4);
            for (const item of order.items) {
                doc.fontSize(10).text(`• ${item.quantity}x ${item.product.name} | Unitário: R$ ${Number(item.unitPrice).toFixed(2)} | Total: R$ ${Number(item.totalPrice).toFixed(2)}`);
                if (item.notes) {
                ...
                for (const variation of item.variations) {
                    for (const option of variation.options) {
                        doc
                            .fontSize(9)
                            .fillColor('#666666')
                            .text(`  - ${variation.group.name}: ${option.option.name} (${Number(option.priceModifier) >= 0 ? '+' : ''}R$ ${Number(option.priceModifier).toFixed(2)})`);
                        doc.fillColor('#000000');
                    }
                }
                doc.moveDown(0.2);
...
async function getOrdersReportSummary(companyId, fromDate, toDate) {
    let createdAtFilter;
...
async function assertInternalCustomerCanReceiveOrder(internalCustomerId) {
